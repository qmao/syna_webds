 ...
 */
export class FsUploader extends ToolbarButton {
  /**
   * Construct a new file browser buttons widget.
   */
  constructor(options: FsUploader.IOptions) {
    super({
      icon: fileUploadIcon,
      onClick: () => {
        this._input.click();
  ...
  private _onInputChanged = () => {
    ...
    void Promise.all(pending).then(values => {
      console.log(values);
	  this._stateChanged.emit(files[0].name);
    }).catch(error => {
  ...
  private _stateChanged = new Signal<FsUploader, string>(this);

  public get stateChanged(): ISignal<FsUploader, string> {
...
/**
 * The namespace for FsUploader class statics.
 */
export namespace FsUploader {
  /**
   * The options used to create an FsUploader.
   */
