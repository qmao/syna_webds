...
class RandomDataModel extends DataModel {
  ...
  update = async () => {
    ...
    // POST request
    const dataToSend = { disable: [17], enable: [18] };
    try {
      ...
      const reply2 = JSON.stringify(reply);
      let obj = JSON.parse(reply2);
      //console.log(obj);
...
let model4 = new RandomDataModel(19, 37);
...
class MyStackedPanel extends StackedPanel {
...
function createWrapper(content: Widget, title: string): Widget {
  let wrapper = new MyStackedPanel();
...
/**
 * Initialization data for the image_heatmap extension.
 */
const plugin: JupyterFrontEndPlugin<void> = {
  id: 'image_heatmap:plugin',
  autoStart: true,
  optional: [ILauncher],
  ...
  activate: (
    ...
    console.log('JupyterLab extension image_heatmap is activated!');
